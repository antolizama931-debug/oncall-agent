...
这个项目帮助技术人员处理线上故障。它会接收并合并异常通知，收集错误记录和运行数据，查询以前的事故与处理方法，提出需要验证的可能原因，并记录审批、处理和恢复检查过程。

当前公开版本只进行安全演练，不会操作真实企业服务器。模型给出的原因属于待验证判断，不会被表述成已经证实的事实。

> 第一次了解项目，请先阅读：[5 分钟看懂 OnCall Agent](docs/项目快速入门.md)。不需要先学习运维、后端或 AI 专业知识。

## 项目文档

- [5 分钟快速入门](docs/项目快速入门.md)：只解释项目用途、一个故障案例、现有能力、页面操作和八个必要术语。**第一次只读这份。**
- [项目技术说明书](docs/项目技术说明书.md)：读完快速入门后，再用它理解架构、数据流、安全、测试和部署。
- [大厂面试速查手册](docs/大厂面试速查手册.md)：包含 30 秒/2 分钟/5 分钟介绍、55 个高频问答、系统设计追问、STAR 表达和七天学习计划。

建议阅读顺序：5 分钟快速入门 → 实际操作一次事故 → 技术说明书 → 面试速查手册。不要从技术说明书开始阅读。
